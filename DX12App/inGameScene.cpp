...
#ifdef STANDALONE
	BuildCarObject({ -306.5f, 3.0f, 253.7f }, { 0.0f, 0.707107f, 0.0f, -0.707107f },  0, true, cmdList, physics, 0);
#else
	int playerCount = 0;
...
void InGameScene::LoadWorldMap(ID3D12GraphicsCommandList* cmdList, const std::shared_ptr<BulletWrapper>& physics, const std::string& path)
{
	...
	while (fgets(buf, 250, file))
	{
		...
		obj->LoadConvexHullShape(convexObjPath + L"_Convex_Hull.obj", btVector3(0, 0, 0), physics);
