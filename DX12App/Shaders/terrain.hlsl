...
PixelOut PS(DsOut din)
{
    ...
    if (PosS.x < 0.0f || PosS.x > 1.0f || PosS.z < 0.0f || PosS.z > 1.0f || PosS.y < 0.0f || PosS.y > 1.0f || idx == -1)
        directLight = ComputeLighting(gLights, gMat, normalize(din.NormalW), din.PosW, view, shadowFactorOut, gRimLightOn, gNumLights);
    else
    {
        directLight = ComputeLighting(gLights, gMat, normalize(din.NormalW), din.PosW, view, shadowFactor, gRimLightOn, gNumLights);
    }
